...
/**
 * Synthetic top-of-panel section listing the N waypoints closest to the player's current
 * tile (computed by {@link com.waypointer.service.NearbyComputer}). Visually identical to
 * {@link PinnedSection} apart from the header label. Rows are drag-disabled.
 */
public class NearbySection extends JPanel
{
        ...
        Function<Waypoint, Category> categoryLookup)
    {
        ...
        JLabel countLabel = new JLabel("(" + nearby.size() + ")");
        ...
        for (Waypoint w : nearby)
        {
            ...
            WaypointRow row = new WaypointRow(
                ...
                () -> onRowAction.accept(w, CategorySection.RowAction.EXPORT),
                () -> onRowAction.accept(w, CategorySection.RowAction.EXPORT_FILE),
                spriteManager,
                originName);
