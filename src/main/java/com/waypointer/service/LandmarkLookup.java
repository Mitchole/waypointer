...
/**
 * Reads bank/altar/anvil/apothecary TSVs (vendored from shortest-path under BSD 2-Clause)
 * and exposes a tile -> landmark name lookup. Null if no entry.
 */
@Slf4j
@Singleton
public class LandmarkLookup
{
    ...
    @Inject
    public LandmarkLookup()
    {
        for (String res : RESOURCES)
        {
            loadResource(res);
        }
        log.info("LandmarkLookup loaded {} tile->name entries", nameByPacked.size());
    }

    /** @return landmark name at this packed tile, or null if no entry. */
    @Nullable
    ...
    {
        try (InputStream in = LandmarkLookup.class.getResourceAsStream(resourcePath))
        {
            if (in == null)
            {
